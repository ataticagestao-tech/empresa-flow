-- =============================================================================
-- FIX URGENTE: status_conciliacao quebrou em prod
-- =============================================================================
-- Erro 23514 em movimentacoes_status_conciliacao_check ao chamar
-- criar_venda_atomica. A constraint original aceita
-- ('pendente','conciliado','divergente','ignorado') com default 'pendente',
-- mas em prod algo foi alterado.
--
-- Este script:
-- 1. Re-garante o default e a check constraint corretos
-- 2. Atualiza criar_venda_atomica e atualizar_venda_atomica pra inserir
--    status_conciliacao = 'pendente' EXPLICITO (não depende mais do default)
-- =============================================================================

-- 1. Garantir default e constraint corretos
ALTER TABLE public.movimentacoes
  ALTER COLUMN status_conciliacao SET DEFAULT 'pendente';

ALTER TABLE public.movimentacoes
  DROP CONSTRAINT IF EXISTS movimentacoes_status_conciliacao_check;

ALTER TABLE public.movimentacoes
  ADD CONSTRAINT movimentacoes_status_conciliacao_check
  CHECK (status_conciliacao IN ('pendente','conciliado','divergente','ignorado'));

-- Backfill: se algum registro existente tem valor inválido, normaliza pra 'pendente'
UPDATE public.movimentacoes
   SET status_conciliacao = 'pendente'
 WHERE status_conciliacao IS NULL
    OR status_conciliacao NOT IN ('pendente','conciliado','divergente','ignorado');

-- =============================================================================
-- 2. Recriar criar_venda_atomica com status_conciliacao explícito
-- =============================================================================
...
-- =============================================================================
-- 3. Recriar atualizar_venda_atomica com status_conciliacao explícito
-- =============================================================================
