-- ============================================================
-- FIX: conciliar_lote estourava FK quando regra de Alta Confianca
-- apontava pra chart_of_accounts.id que nao existe mais (sequela
-- do reset de plano de contas de 29/04: categorias deletadas +
-- recriadas com IDs novos deixaram regras com action_value stale).
--
-- Comportamento novo:
--   - Antes de criar CP/CR, valida se v_account_id existe E
--     pertence ao p_company_id.
--   - Se invalido -> seta v_account_id := NULL (CP/CR cai como
--     "sem categoria", aparece no banner de pendencias pra
--     reclassificacao manual).
--   - Item nao falha mais; o lote inteiro conclui.
--
-- Resto da RPC permanece identico ao da migration
-- 20260415150000_conciliar_lote_preenche_fk_e_backfill.sql.
-- ============================================================
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- ✱ NOVO: validar conta contábil contra chart_of_accounts da empresa ✱
      -- Se a regra/sugestão aponta pra um id que não existe mais ou pertence a
      -- outra empresa, zera v_account_id em vez de estourar FK. Item entra como
      -- "sem categoria" e vira pendência de reclassificação.
      IF v_account_id IS NOT NULL THEN
        ...
        IF v_account_valid IS NULL THEN
          v_account_id := NULL;
        END IF;
        v_account_valid := NULL;  -- reseta pro proximo loop
      END IF;
