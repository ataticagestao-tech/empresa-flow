// agente-tool-buscar_socio — busca sócio/administrador da empresa
...
serve(async (req) => {
    ...
    try {
        ...
        const { data, error } = await service.rpc("agente_buscar_socio", { p_company_id: empresa_id, p_termo: termo });
        if (error) return j({ error: error.message }, 500);

        return j({ ok: true, qtd: (data || []).length, socios: data || [] });
