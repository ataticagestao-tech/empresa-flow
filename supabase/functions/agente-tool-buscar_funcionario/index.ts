// agente-tool-buscar_funcionario — busca funcionário por nome ou CPF
...
serve(async (req) => {
    ...
    try {
        ...
        const { data, error } = await service.rpc("agente_buscar_funcionario", { p_company_id: empresa_id, p_termo: termo });
        if (error) return j({ error: error.message }, 500);

        return j({ ok: true, qtd: (data || []).length, funcionarios: data || [] });
