...
<body>
  ...
  // ─── Exportar PDF (ABNT NBR 14724) ────────────────────────────────
  const exportarPDF = () => {
    if (filteredContas.length === 0) {
      toast.error('Nenhum titulo na lista para exportar')
      return
    }

    // Página A4 e margens ABNT (sup 3, esq 3, inf 2, dir 2 cm)
    ...
    const FONT = 'times'
    const LH12 = 7    // entrelinha 1,5 para corpo 12pt (~6,77mm; arredondado)
    const LH10 = 5.8  // entrelinha 1,0 para 10pt (legendas/quadro)
    const INDENT = 12.5 // recuo de paragrafo 1,25 cm

    // Resolve filtros para texto legivel
    const sectorNome = sectorFilter === 'todos'
      ? 'todos os setores'
      : (centrosCusto.find(c => c.id === sectorFilter)?.descricao || 'nao identificado')
    const statusLabel: Record<string, string> = {
      todos: 'todos os status',
      aberto: 'titulos em aberto',
      vencidos: 'titulos vencidos',
      pagos: 'titulos pagos',
    }
    const statusNome = statusLabel[statusFilter] || statusFilter
    const periodoTxt = (dateFrom && dateTo)
      ? `de ${format(parseISO(dateFrom), 'dd/MM/yyyy')} a ${format(parseISO(dateTo), 'dd/MM/yyyy')}`
      : 'sem restricao de datas'
    const dataEmissaoExt = format(new Date(), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })
    const cidadeSede = selectedCompany?.endereco_cidade || ''
    const ufSede = selectedCompany?.endereco_estado || ''
    const localData = `${cidadeSede ? cidadeSede + (ufSede ? '/' + ufSede : '') + ', ' : ''}${dataEmissaoExt}.`

    const totalGeral = filteredContas.reduce((s, cp) => s + saldo(cp), 0)
    const totalAberto = filteredContas
      .filter(c => c.status !== 'pago' && c.status !== 'cancelado')
      .reduce((s, cp) => s + saldo(cp), 0)
    const totalPagoVal = filteredContas
      .filter(c => c.status === 'pago')
      .reduce((s, cp) => s + (cp.valor_pago || cp.valor), 0)
    const qtdAbertos = filteredContas.filter(c => c.status !== 'pago' && c.status !== 'cancelado').length
    const qtdPagos = filteredContas.filter(c => c.status === 'pago').length
    ...
    const desenharNumeroPagina = () => {
      ...
      // ABNT: numero no canto superior direito, a 2cm da borda direita e 2cm da borda superior
    ...
    const escreverParagrafo = (texto: string, y: number, opts?: { recuo?: boolean; bold?: boolean; justify?: boolean }) => {
      doc.setFont(FONT, opts?.bold ? 'bold' : 'normal')
      doc.setFontSize(12)
      doc.setTextColor(0, 0, 0)
      const recuoX = opts?.recuo ? INDENT : 0
      const linhas: string[] = doc.splitTextToSize(texto, contentW - recuoX) as string[]
      const primeira = linhas[0]
      const resto = linhas.slice(1)
      y = garantirEspaco(y, LH12)
    ...
    // Titulo principal — caixa alta, negrito, centralizado, fonte 12 (NBR 14724 §5.2.2)
    doc.setFont(FONT, 'bold')
    doc.setFontSize(12)
    doc.setTextColor(0, 0, 0)
    doc.text('RELATORIO DE CONTAS A PAGAR', W / 2, y, { align: 'center' })
    y += LH12

    // Subtitulo: razao social
    if (selectedCompany?.razao_social) {
      doc.setFont(FONT, 'bold')
      doc.setFontSize(12)
      doc.text(selectedCompany.razao_social.toUpperCase(), W / 2, y, { align: 'center' })
      y += LH12
    }
    if (selectedCompany?.cnpj) {
      doc.setFont(FONT, 'normal')
      doc.setFontSize(10)
      doc.text(`CNPJ: ${selectedCompany.cnpj}`, W / 2, y, { align: 'center' })
      y += LH10
    }
    y += LH12 // espaço duplo apos titulo (NBR 14724)

    // 1 INTRODUCAO
    y = escreverParagrafo('1 INTRODUCAO', y, { bold: true })
    y += 2
    const intro = `O presente documento consolida a relacao dos titulos a pagar registrados no sistema de gestao financeira da empresa, ` +
      `tendo por finalidade dar publicidade interna as obrigacoes assumidas e a respectiva agenda de vencimentos. ` +
      `Os dados foram extraidos com os seguintes filtros: periodo de vencimento ${periodoTxt}; ` +
      `escopo ${statusNome}; centro de custo ${sectorNome}` +
      (searchTerm ? `; termo de busca aplicado "${searchTerm}"` : '') +
      `. O total de obrigacoes apuradas e de ${formatBRL(totalGeral)}, distribuido em ${qtdAbertos} titulo(s) em aberto ` +
      `(${formatBRL(totalAberto)}) e ${qtdPagos} titulo(s) ja quitado(s) (${formatBRL(totalPagoVal)}).`
    y = escreverParagrafo(intro, y, { recuo: true })
    y += LH12 // separacao de secao

    // 2 RELACAO DE TITULOS
    y = escreverParagrafo('2 RELACAO DE TITULOS', y, { bold: true })
    y += 2

    // Legenda do quadro (acima do quadro, fonte 10 — NBR 14724 §5.6)
    y = garantirEspaco(y, LH10 * 2)
    doc.setFont(FONT, 'bold')
    doc.setFontSize(10)
    doc.text('Quadro 1 — Relacao analitica de titulos a pagar', ML, y)
    y += LH10

    // Cabecalho da tabela
    const cols = [
      { label: 'N.', w: 10, align: 'left' as const },
      { label: 'Vencimento', w: 24, align: 'left' as const },
      { label: 'Credor / Descricao', w: 70, align: 'left' as const },
      { label: 'Categoria', w: 25, align: 'left' as const },
      { label: 'Status', w: 18, align: 'left' as const },
      { label: 'Valor (R$)', w: 0, align: 'right' as const }, // ocupa o restante
    ]
    const usados = cols.reduce((s, c) => s + c.w, 0)
    cols[cols.length - 1].w = contentW - usados
    ...
    const desenharCabecalhoTabela = (yy: number) => {
      ...
      // Linhas de fechamento ABNT (sup e inf do cabecalho)
      doc.line(ML, yy - 0.5, ML + contentW, yy - 0.5)
      let xx = ML
      for (const c of cols) {
        const tx = c.align === 'right' ? xx + c.w - 1 : xx + 1
        doc.text(c.label, tx, yy + 4, { align: c.align })
        xx += c.w
      }
      yy += 5.5
      doc.line(ML, yy - 0.5, ML + contentW, yy - 0.5)
    ...
    for (const cp of filteredContas) {
      nLinha += 1
      const credor = (cp.credor_nome || '—').trim()
      const descr = (cp.descricao || '').trim()
      const linhaCD = descr ? `${credor} — ${descr}` : credor
      const venc = format(parseISO(cp.data_vencimento), 'dd/MM/yyyy')
      const cat = inferCategoria(cp)
      const st = cp.status === 'pago' ? 'Pago'
        : cp.status === 'cancelado' ? 'Cancel.'
        : (cp.data_vencimento < format(new Date(), 'yyyy-MM-dd') ? 'Vencido' : 'Aberto')
      const valor = formatBRL(saldo(cp) > 0 ? saldo(cp) : (cp.valor_pago || cp.valor))

      // Quebra do credor/descricao em multiplas linhas se necessario
      const cdLines = doc.splitTextToSize(linhaCD, cols[2].w - 2) as string[]
      const altura = Math.max(LH10, cdLines.length * LH10)
      y = garantirEspaco(y, altura + 1)
      if (y === MT) {
        // Acabou de virar a pagina — redesenhar cabecalho
        y = desenharCabecalhoTabela(y)
      }

      let xx = ML
      // N.
      doc.text(String(nLinha), xx + 1, y + 4)
      xx += cols[0].w
      // Vencimento
      doc.text(venc, xx + 1, y + 4)
      xx += cols[1].w
      // Credor / Descricao (multilinhas)
      cdLines.forEach((ln, i) => {
        doc.text(ln, xx + 1, y + 4 + i * LH10)
      })
      xx += cols[2].w
      // Categoria
      doc.text(cat, xx + 1, y + 4)
      xx += cols[3].w
      // Status
      doc.text(st, xx + 1, y + 4)
      xx += cols[4].w
      // Valor
      doc.text(valor, xx + cols[5].w - 1, y + 4, { align: 'right' })
    ...
    // Linha de fechamento da tabela (ABNT)
    doc.setDrawColor(0, 0, 0)
    doc.setLineWidth(0.3)
    doc.line(ML, y, ML + contentW, y)
    y += LH10
    ...
    doc.setFont(FONT, 'normal')
    doc.setFontSize(10)
    doc.text(`Fonte: sistema de gestao financeira (extracao em ${format(new Date(), 'dd/MM/yyyy HH:mm')}).`, ML, y)
    y += LH12

    // 3 SINTESE FINANCEIRA
    y = garantirEspaco(y, LH12 * 6)
    y = escreverParagrafo('3 SINTESE FINANCEIRA', y, { bold: true })
    y += 2

    const sintese = `Considerando o universo de ${filteredContas.length} titulo(s) listado(s) na secao anterior, ` +
      `apurou-se um saldo total de obrigacoes equivalente a ${formatBRL(totalGeral)}. ` +
      `Desse montante, ${formatBRL(totalAberto)} permanecem em aberto, distribuidos em ${qtdAbertos} titulo(s), ` +
      `enquanto ${formatBRL(totalPagoVal)} ja foram efetivamente liquidados (${qtdPagos} titulo(s)). ` +
      `Recomenda-se a conferencia periodica desta relacao com os extratos bancarios e demais lancamentos contabeis.`
    y = escreverParagrafo(sintese, y, { recuo: true })
    y += LH12

    // Local e data — alinhamento a direita, padrao de documento formal
    ...
    // Salvar
    const nome = `relatorio-contas-a-pagar-${format(new Date(), 'yyyy-MM-dd')}.pdf`
    doc.save(nome)
  }
