...
export default function PlanoContas() {
  ...
  const [newConta, setNewConta] = useState({ code: "", name: "", account_type: "expense", account_nature: "devedora", parent_code: "", show_in_dre: true });
  ...
  const tree = useMemo((): TreeNode[] => {
    ...
    return grupos.map(g => ({
      ...
      filhos: subgrupos
        ...
        .map(s => ({
          ...s,
          filhos: analiticas.filter(a => a.code.startsWith(s.code + ".")),
        })),
    }));
  }, [filteredContas]);
  ...
  const expandAll = () => {
    ...
    contas.forEach(c => { if (c.level <= 2) all.add(c.code); });
    setExpandidos(all);
  };
  ...
  const handleAddConta = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        show_in_dre: newConta.show_in_dre, parent_id: parent?.id || null,
        status: "active", accepts_manual_entry: level === 3,
      };
      ...
      setShowForm(false);
      setNewConta({ code: "", name: "", account_type: "expense", account_nature: "devedora", parent_code: "", show_in_dre: true });
          ...
          <div className="border border-[#ccc] rounded-lg overflow-hidden">
            ...
            <div className="p-5 bg-white space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="flex flex-col gap-1"><label className={LB}>Conta Pai (código)</label>
                    ...
                    {expandidos.has(sub.code) && sub.filhos.map(analitica => {
                      ...
                      return (
                        <div key={analitica.id}
                          className="bg-white pl-16 pr-4 py-2 flex items-center gap-3 border-b border-[#f0f0f0] hover:bg-[#fafafa]">
