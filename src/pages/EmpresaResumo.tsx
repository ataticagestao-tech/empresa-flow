...
export default function EmpresaResumo() {
  ...
  const exportarFichaPDF = async () => {
    ...
    const logo = await carregarLogoEmpresa((company as any).logo_url);
    const blob = gerarFichaEmpresaPDF({
      company,
      qsa,
      regimeLabels,
      cor_primaria: "#059669",
    ...
    const slug = String(company.razao_social || "empresa").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60);
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "ficha-cadastral-" + slug + "-" + format(new Date(), "yyyy-MM-dd") + ".pdf";
